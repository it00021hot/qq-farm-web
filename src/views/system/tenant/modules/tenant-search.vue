      ...
      <NCollapseItem :title="$t('common.search')" name="tenant-search">
        <NForm ref="formRef" :model="model" label-placement="left" :label-width="80">
          <NGrid responsive="screen" item-responsive>
            <NFormItemGi span="24 s:12 m:6" :label="$t('page.system.tenant.keyword')" path="keyword">
              <NInput v-model:value="model.keyword" :placeholder="$t('common.keywordSearch')" />
            </NFormItemGi>
            <NFormItemGi span="24 s:12 m:6" :label="$t('page.system.tenant.status')" path="status">
              <NSelect
                v-model:value="model.status"
                :options="statusOptions"
                clearable
                :placeholder="$t('page.system.tenant.status')"
              />
            </NFormItemGi>
